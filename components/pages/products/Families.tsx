...
/**
 * app/products/families/page.tsx  (REFACTORED — TOKEN inline-style system)
 *
 * Fully re-skinned to match the site-wide TOKEN design system used by
 * AllProducts.tsx, ProductRequests.tsx, and CMSLayout.
 *
 * All business logic (Firestore, TDS generation, Cloudinary, application
 * propagation) is 100% preserved from the original.
 */
...
type SpecGroupDoc      = { id: string; name: string; items?: { label: string }[]; isActive?: boolean };
type ApplicationDoc    = { id: string; title?: string; name?: string; imageUrl?: string; websites?: string[] };
...
function UsagePill({ label, active, onClick }: { label: ProductUsage; active: boolean; onClick: () => void }) {
...
// ─── Simple searchable multi-select dropdown ──────────────────────────────────
      ...
      {open && (
        <>
          <motion.div
            ...
            exit={{   opacity: 0 }}
            onClick={onClose}
            style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.45)", backdropFilter: "blur(4px)", zIndex: 400 }}
          />
          <div style={{ position: "fixed", inset: 0, zIndex: 401, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, pointerEvents: "none" }}>
            <motion.div
              key="del-dialog"
            ...
            >
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
                <div style={{ width: 52, height: 52, borderRadius: 14, background: TOKEN.dangerBg, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Trash2 size={22} color={TOKEN.danger} />
                </div>
              </div>
              <p style={{ fontSize: 16, fontWeight: 700, textAlign: "center", color: TOKEN.textPri, margin: "0 0 8px" }}>{title}</p>
              <p style={{ fontSize: 13, textAlign: "center", color: TOKEN.textSec, margin: "0 0 24px", lineHeight: 1.6 }}>{description}</p>
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={onClose} disabled={loading} style={{ ...outlineBtn, flex: 1, justifyContent: "center", padding: "11px 0", borderRadius: 12 }}>Cancel</button>
                <button onClick={onConfirm} disabled={loading} style={{ flex: 2, padding: "11px 0", borderRadius: 12, border: "none", background: TOKEN.danger, color: "#fff", fontSize: 13.5, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                  {loading ? <Loader2 size={16} className="animate-spin" /> : <><Trash2 size={15} /> Delete</>}
                </button>
              </div>
            </motion.div>
          </div>
...
function FamilyCard({
  ...
  const itemCount = Array.isArray(family.specs)
    ...
    : 0;
  const appCount = Array.isArray(family.applications) ? family.applications.length : 0;
  ...
  return (
    ...
    >
      {/* Image area */}
      <div style={{ position: "relative", aspectRatio: "4/3", background: TOKEN.bg, overflow: "hidden" }}>
        {img ? (
          <img src={img} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt={family.title ?? ""} />
        ) : (
          <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <FolderPlus size={32} style={{ opacity: 0.2, color: TOKEN.textSec }} />
          </div>
        )}
        {/* Checkbox */}
        <div
        ...
        <div style={{ position: "absolute", top: 10, right: 10, display: "flex", gap: 6 }}>
          <button
            ...
            style={{ ...iconBtnStyle, background: "rgba(255,255,255,0.9)", backdropFilter: "blur(4px)", width: 28, height: 28, borderRadius: 8 }}
          >
            <Pencil size={12} />
          </button>
          <button
            onClick={(e) => { e.stopPropagation(); onDelete(); }}
            style={{ ...iconBtnStyle, background: "rgba(255,255,255,0.9)", backdropFilter: "blur(4px)", width: 28, height: 28, borderRadius: 8, color: TOKEN.danger }}
          >
            <Trash2 size={12} />
          </button>
        </div>
      ...
      <div style={{ padding: "12px 14px", display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
          <h3 style={{ margin: 0, fontSize: 13, fontWeight: 800, color: TOKEN.textPri, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>
        ...
        {family.description && (
          <p style={{ margin: 0, fontSize: 11, color: TOKEN.textSec, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontStyle: "italic" }}>
            {family.description}
          </p>
        )}
        ...
        {family.productUsage && family.productUsage.length > 0 && (
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {family.productUsage.map((u) => {
              ...
              return (
                <span key={u} style={{ fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 999, border: `1px solid ${c.border}`, background: c.bg, color: c.color, textTransform: "uppercase" }}>
                  {u}
                </span>
              );
            })}
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {appCount > 0 && (
            <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 700, color: TOKEN.textSec, textTransform: "uppercase" }}>
              <Briefcase size={10} />
              {appCount} App{appCount !== 1 ? "s" : ""}
            </span>
          )}
          {groupCount > 0 && (
            <span style={{ fontSize: 10, fontWeight: 700, color: TOKEN.textSec, textTransform: "uppercase" }}>
...
function ProductFamiliesContent() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const payload: any   = { title: normalisedTitle, specs: selectedSpecsForSave, productUsage, applications: selectedApplicationIds, updatedAt: serverTimestamp() };
      ...
      if (editId) {
        ...
        const ref  = await addDoc(collection(db, "productfamilies"), { ...payload, isActive: true, createdAt: serverTimestamp() });
        familyDocId = ref.id;
      }
      ...
      if (selectedApplicationIds.length > 0 && normalisedTitle) {
        try {
          const productsSnap = await getDocs(query(collection(db, "products"), where("productFamily", "==", normalisedTitle)));
        ...
        const specGroupsForTemplate = selectedSpecGroupIds
          .map((gid) => {
            ...
            const items = (group?.items ?? [])
              ...
              .filter((i) => { if (!i.label) return false; return selectedItemIds.has(buildSpecItemId(gid, i.label)); });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(Array.from(selectedIds).map((id) => deleteDoc(doc(db, "productfamilies", id))));
    ...
    <div style={{ width: "100%", maxWidth: 1400, margin: "0 auto" }}>
      ...
      <div style={{ paddingTop: 20, paddingBottom: 20 }}>
        ...
        <p style={{ margin: "4px 0 0", fontSize: 13, color: TOKEN.textSec }}>
          Create and manage product families, specs, and application assignments. A blank TDS template is auto-generated on each save.
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,2fr)", gap: 24, alignItems: "start" }}
        ...
        {/* ════ FORM COLUMN ════ */}
        <div style={{ position: "sticky", top: 80, zIndex: 10, maxHeight: "calc(100vh - 100px)", overflowY: "auto" }}>
          <div style={sectionCard}>
            {/* Form header */}
            <div style={{ padding: "16px 20px", borderBottom: `1px solid ${TOKEN.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 800, color: TOKEN.textPri, textTransform: "uppercase", letterSpacing: "0.04em" }}>
                {editId ? "Edit Family" : "Add Family"}
              </p>
              {editId && (
                <button type="button" onClick={resetForm} style={{ ...outlineBtn, fontSize: 11 }}>
            ...
            <form onSubmit={handleSubmit} style={{ padding: "20px", display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              {/* Title */}
              <div>
                <label style={labelStyle}>Title <span style={{ color: TOKEN.danger }}>*</span></label>
                <input
                  value={title}
                  onChange={(e) => setTitle(e.target.value.toUpperCase())}
                  placeholder="E.G. RECESSED LIGHTS"
                  style={{ ...inputStyle, borderColor: !title.trim() ? `${TOKEN.danger}60` : TOKEN.border }}
                />
                {!title.trim() && <p style={{ margin: "5px 0 0", fontSize: 10, color: TOKEN.danger, fontWeight: 700 }}>Title is required</p>}
              </div>


              {/* Product Usage */}
              <div>
                <label style={labelStyle}>Product Usage <span style={{ opacity: 0.5 }}>(optional)</span></label>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {PRODUCT_USAGE_OPTIONS.map((u) => (
                    <UsagePill
                      ...
                      onClick={() => setProductUsage((p) => p.includes(u) ? p.filter((v) => v !== u) : [...p, u])}
                    />
                  ))}
                </div>
                {productUsage.length === 0 && (
                  <p style={{ margin: "6px 0 0", fontSize: 10, color: TOKEN.textSec, fontWeight: 600 }}>Tagging usage helps filter families in the product form</p>
                )}
              </div>

              {/* Image dropzone */}
              <div>
                <label style={labelStyle}>Image <span style={{ opacity: 0.5 }}>(optional)</span></label>
                <div
                  {...getRootProps()}
                  style={{
                ...
                >
                  ...
                  {previewUrl ? (
                    <div style={{ position: "relative", width: "100%" }}>
                      <img src={previewUrl} style={{ width: "100%", aspectRatio: "16/9", objectFit: "cover", display: "block" }} alt="Preview" />
                      <button
                        type="button"
                        onClick={(e) => { e.stopPropagation(); setPreviewUrl(""); setImageFile(null); }}
                        style={{ position: "absolute", top: 8, right: 8, width: 24, height: 24, borderRadius: 6, border: "none", background: TOKEN.danger, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
                      >
                        <X size={12} />
                      </button>
                    </div>
                    ...
                    <div style={{ textAlign: "center" }}>
                      ...
                      <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: TOKEN.textSec }}>Drop image here</p>
                      <p style={{ margin: "2px 0 0", fontSize: 10, color: TOKEN.textSec, opacity: 0.6 }}>or click to browse</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                {selectedApplicationIds.length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
                    {selectedApplicationIds.map((appId) => {
                      ...
                      return (
                        <span key={appId} style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 10, fontWeight: 700, padding: "3px 8px 3px 10px", borderRadius: 6, background: TOKEN.bg, border: `1px solid ${TOKEN.border}`, color: TOKEN.textPri }}>
                          {name.toUpperCase()}
                          <button type="button" onClick={() => handleToggleApplication(appId)} style={{ background: "none", border: "none", cursor: "pointer", color: TOKEN.textSec, padding: 0, display: "flex", lineHeight: 1 }}>
                            <X size={10} />
                          </button>
                        </span>
                      );
                ...
                {selectedApplicationIds.length > 0 && (
                  <p style={{ margin: "5px 0 0", fontSize: 10, color: TOKEN.textSec, fontWeight: 600 }}>Products in this family will be tagged with the selected applications</p>
                )}
              </div>
              ...
              <div>
                ...
                />
                {!canPickSpecs && (
                  <p style={{ margin: "5px 0 0", fontSize: 10, color: TOKEN.textSec, fontWeight: 600 }}>Spec selection unlocks after title is set</p>
                )}
              </div>
              ...
              {selectedSpecGroupIds.length > 0 && (
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, color: TOKEN.textSec, background: TOKEN.bg, border: `1px solid ${TOKEN.border}`, borderRadius: 6, padding: "2px 8px" }}>
                  ...
                  {selectedSpecGroupIds.map((gid) => {
                    ...
                    const labels    = Array.from(new Set((group?.items ?? []).map((i) => i.label).filter(Boolean).map((l) => l.toUpperCase().trim())));
                    ...
                    return (
                      <div key={gid} style={{ border: `1px solid ${TOKEN.border}`, borderRadius: 10, overflow: "hidden" }}>
                        {/* Group header */}
                        <div style={{ padding: "10px 14px", borderBottom: `1px solid ${TOKEN.border}`, background: TOKEN.bg, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                          <div style={{ minWidth: 0 }}>
                            <p style={{ margin: 0, fontSize: 12, fontWeight: 800, color: TOKEN.textPri, textTransform: "uppercase" }}>{groupName}</p>
                            <p style={{ margin: "2px 0 0", fontSize: 10, color: TOKEN.textSec, fontWeight: 600 }}>{selectedSet.size} SELECTED · {labels.length} AVAILABLE</p>
                          </div>
                          <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                            <button type="button" onClick={() => setAllItemsInGroup(gid, true)}  style={{ ...outlineBtn, fontSize: 10, padding: "5px 10px" }} disabled={labels.length === 0}>All</button>
                            <button type="button" onClick={() => setAllItemsInGroup(gid, false)} style={{ ...outlineBtn, fontSize: 10, padding: "5px 10px" }}>Clear</button>
                          </div>
                        </div>
                        {/* Search + items */}
                        <div style={{ padding: "10px 12px", display: "flex", flexDirection: "column", gap: 8 }}>
                          <input
                            ...
                            onChange={(e) => setSpecItemSearch((prev) => ({ ...prev, [gid]: e.target.value.toUpperCase() }))}
                            placeholder="Filter items…"
                            style={{ ...inputStyle, fontSize: 12, padding: "8px 12px" }}
                          />
                          {filtered.length === 0 ? (
                            <p style={{ fontSize: 11, color: TOKEN.textSec, textAlign: "center", padding: "12px 0", background: TOKEN.bg, borderRadius: 8 }}>No items found</p>
                          ) : (
                            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 4, maxHeight: 200, overflowY: "auto" }}>
                              {filtered.map((label) => {
                                ...
                                return (
                                  ...
                                  >
                                    <span style={{ width: 16, height: 16, borderRadius: 4, border: `1.5px solid ${checked ? TOKEN.primary : TOKEN.border}`, background: checked ? TOKEN.primary : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                                      {checked && <Check size={10} color="#fff" />}
                                    </span>
                                    <span style={{ fontSize: 11, fontWeight: 700, color: TOKEN.textSec, textTransform: "uppercase" }}>{label}</span>
                                  </button>
                                );
                              })}
                            </div>
                          ...
                          {(specItemSelections[gid] ?? []).length === 0 && (
                            <p style={{ margin: 0, fontSize: 10, color: TOKEN.danger, fontWeight: 700 }}>Select at least one item for this group</p>
                          )}
                        </div>
                      </div>
                    );
              ...
              {selectedSummary.length > 0 && (
                <div style={{ background: TOKEN.bg, border: `1px solid ${TOKEN.border}`, borderRadius: 10, padding: "12px 14px" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                    <p style={{ margin: 0, fontSize: 11, fontWeight: 800, color: TOKEN.textSec, textTransform: "uppercase" }}>Selection Summary</p>
                    <span style={{ fontSize: 10, fontWeight: 700, color: TOKEN.textSec, background: TOKEN.surface, border: `1px solid ${TOKEN.border}`, borderRadius: 6, padding: "2px 8px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {selectedSummary.map((g) => {
                      ...
                      return (
                        <div key={g.specGroupId} style={{ background: TOKEN.surface, border: `1px solid ${TOKEN.border}`, borderRadius: 8, padding: "10px 12px" }}>
                          ...
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                            {uniqueItems.map((it) => (
                              <span key={it.id} style={{ fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 4, background: TOKEN.bg, border: `1px solid ${TOKEN.border}`, color: TOKEN.textSec, textTransform: "uppercase" }}>
                                {it.name}
                              </span>
                            ))}
                          </div>
              ...
              {/* Submit */}
              <button type="submit" disabled={isSubmitLoading} style={{ ...primaryBtn, opacity: isSubmitLoading ? 0.7 : 1 }}>
                {isSubmitLoading
                  ? <><Loader2 size={16} className="animate-spin" /> Saving…</>
                  : editId ? "Push Update" : <><Plus size={15} /> Create Product Family</>
                }
              </button>
            </form>
          </div>
        ...
        <div>
          ...
          {!loadingFamilies && families.length > 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 20 }}>
              ...
              <div style={{ position: "relative" }}>
                <Search size={15} style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: TOKEN.textSec }} />
                <input
                  type="text"
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  {selectedIds.size > 0 && (
                    <>
                      <button onClick={() => setBulkDeleteOpen(true)} style={{ ...outlineBtn, color: TOKEN.dangerText, borderColor: `${TOKEN.danger}40`, background: TOKEN.dangerBg }}>
          ...
          {/* Grid */}
          {loadingFamilies ? (
            <div style={{ padding: "80px 0", display: "flex", flexDirection: "column", alignItems: "center", gap: 12, color: TOKEN.textSec }}>
              <Loader2 size={28} className="animate-spin" style={{ color: TOKEN.primary }} />
              <p style={{ margin: 0, fontSize: 12, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase" }}>Loading…</p>
            </div>
          ) : filteredFamilies.length === 0 ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 320, border: `2px dashed ${TOKEN.border}`, borderRadius: 16, background: TOKEN.bg, padding: 32, textAlign: "center" }}>
              <div style={{ width: 64, height: 64, borderRadius: 999, background: TOKEN.surface, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
                <FolderPlus size={28} style={{ color: TOKEN.textSec, opacity: 0.3 }} />
              </div>
              <h3 style={{ margin: "0 0 8px", fontSize: 14, fontWeight: 800, color: TOKEN.textPri, textTransform: "uppercase", letterSpacing: "0.06em" }}>
